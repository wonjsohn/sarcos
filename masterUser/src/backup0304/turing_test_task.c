...
*/
 ...
 // read coefficient array from txt. (pos / vel / force)
  FILE *fp = fopen("src/regressionCoeff.txt", "r"); 
  if (!fp) {
...
//task: find a coefficients posCoef, velCoef, loadCoef really fast.

//joint_des_state[R_EB].uff =(pos2DArray[0]+ pos2DArray[1]*omega0 + pos2DArray[2]*omega1)*0.1 + gravity ; // + (loadCoef*torque)*1.0; //+ gravity; 
joint_des_state[R_EB].uff =pos2DArray[0] + (pos2DArray[1]*omega0 + pos2DArray[2]*omega1)*1.2 + gravity+1.0 ; // + (loadCoef*torque)*1.0; //+ gravity;   // no gravity since it should have been taken care in playback - record stage. 
