...
/* global variables   */
char* g_py2robot = NULL;
char* g_robot2py = NULL;
char *homePath  ;   // environmental variable to use relative path. 
char *homePath_copy  ;   // environmental variable to use relative path. 
struct stat statbuf;   

/* local functions */
static int  init_python_task(void);
static int  run_python_task(void);
static int  change_python_task(void);
...
*****************************************************************************/
void
add_python_task( void )

{
  int i, j;
  
  addTask("Python Task", init_python_task, 
	  run_python_task, change_python_task);
...
init_python_task(void)
{
 ...
 printf("buffer_size: %d\n\n", statbuf.st_size);
    ...
    if((fd_i = open(py2robot_FilePath, O_RDWR)) == -1)
	{
   	...
   	g_py2robot = mmap(NULL, 1024, PROT_READ | PROT_WRITE, MAP_SHARED, fd_i, 0);
    
    if (g_py2robot == MAP_FAILED) {
        close(fd_i);
   ...
   // if((fd_o = open(robot2fpga_FilePath, O_RDWR)) == -1)
    ...
    g_robot2py = mmap(NULL, 1024, PROT_READ | PROT_WRITE, MAP_SHARED, fd_o, 0);
    
    if (g_robot2py == MAP_FAILED) {
        close(fd_o);
        perror("Error mmapping the file");
        exit(EXIT_FAILURE);
    }
	...
	system("/Network/Servers/duerer/guest/prog/masterUser/src/py/py2robot.py &");
  //end code
...
run_python_task(void)
{
  ...
  float currentAngularVel;
    ...
    sscanf(g_py2robot, "%f", &tmpdata); // read elbow Force from python?
