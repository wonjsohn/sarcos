...
static SL_DJstate joint_setpoint[N_DOFS+1];
static int subtask;
...
static Matrix local_Alinkdofs_des[N_LINKS+1];
...

{
  ...
  if (firsttime) {
    ...
    for (i=0; i<=N_DOFS; ++i)
      local_Alinkdofs_des[i] = my_matrix(1,4,1,4);
  ...
  go_cart_target_wait(&setpoint-1,jstatus, 2.0);
  ...
  for (i=1; i<=N_DOFS; ++i)
    target[i] = joint_setpoint[i] = joint_des_state[i];
  ...
  // ready to go
  answer = -999;
  get_int("Type 1 to start the task or hit return to stop ...",
	  answer,&answer);
...

{
  ...
  case NEW_TARGET: // ---------------------------------------------------------
    ...
    for (i=1; i<=N_DOFS; ++i)
      q_state[i] = joint_setpoint[i];

    for (i=1; i<=N_DOFS; ++i) {
      target[i].th   = joint_setpoint[i].th;
      target[i].thd  = 0.0;
      target[i].thdd = 0.0;
      target[i].uff  = 0.0;
    }
  ...
  case CHECK_TARGET: // check the target numerically -----------------------------------
	...
	break;
      ...
      if (q_state[R_EB].th  <  0.5  ||
	  q_state[R_EB].th  >  2.1  ||
	  q_state[R_HR].th  < -0.5  ||
	  q_state[R_SAA].th > -0.1) 
	break;
      ...
      for (i=1; i<=N_CART; ++i)
	c_state.x[i] = local_link_pos_des[HAND][i];

      // if we reach much higher than the shoulder, or too much accross the midline, stop
      if (c_state.x[_Z_] > link_pos[SHOULDER][_Z_]+0.1 || c_state.x[_X_] < -0.1)
	break;
    ...
    for (i=1; i<=N_CART; ++i)
      ball[i] = ctarget.x[i];
    sendUserGraphics("ball",&(ball[_X_]), N_CART*sizeof(float));
  ...
  case MOVE_OUT: // move out with IK ---------------------------------------------
    ...
    duration -= time_step;
    if (duration < time_step || target[R_EB].th  <  0.5) { 
      subtask = MOVE_IN;
  ...
  case MOVE_IN: // move back with joint space motion ---------------------------------
    ...
    for (i=1; i<=N_DOFS; ++i) {
      minJerkNextStep(target[i].th, 
		      ...
		      target[i].thdd,
		      joint_setpoint[i].th,
		      joint_setpoint[i].thd,
		      joint_setpoint[i].thdd,
		      duration,
		      time_step,
		      &(target[i].th), 
		      &(target[i].thd),
  ...
  for (i=1; i<=N_DOFS; ++i) {
    ...
    if (i >= R_TL) {
      joint_des_state[i].th   = joint_setpoint[i].th;
