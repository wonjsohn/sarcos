...
/* global variables   */
char* g_py2robot_position_control = NULL;
char* g_robot2py_position_control = NULL;
char *homePath  ;   // environmental variable to use relative path. 
char *homePath_copy  ;   // environmental variable to use relative path. 
struct stat statbuf;   

/* local functions */
static int  init_python_task_position_control(void);
static int  run_python_task_position_control(void);
static int  change_python_task_position_control(void);
...
*****************************************************************************/
void
add_python_task_position_control( void )

{
  int i, j;
  
  addTask("Python Task (Position Control)", init_python_task_position_control, 
	  run_python_task_position_control, change_python_task_position_control);
...
void py_position_control_err_sys(const char* x) 
{ 
    perror(x); 
    exit(1); 
}
...
static int 
init_python_task_position_control(void)
{
  int j, i;
  int ans;
 ...
 printf("buffer_size: %d\n\n", statbuf.st_size);
    ...
    if((fd_i = open(py2robot_FilePath, O_RDWR)) == -1)
	{
   	...
   	g_py2robot_position_control = mmap(NULL, 1024, PROT_READ | PROT_WRITE, MAP_SHARED, fd_i, 0);
    
    if (g_py2robot_position_control == MAP_FAILED) {
        close(fd_i);
   ...
   // if((fd_o = open(robot2fpga_FilePath, O_RDWR)) == -1)
    ...
    g_robot2py_position_control = mmap(NULL, 1024, PROT_READ | PROT_WRITE, MAP_SHARED, fd_o, 0);
    
    if (g_robot2py_position_control == MAP_FAILED) {
        close(fd_o);
        perror("Error mmapping the file");
        exit(EXIT_FAILURE);
    }
    ...
    if(g_py2robot_position_control != NULL)
    {	
        printf("Wrapper has created a MMAP for file 'py2robot.dat'\n");
    }

    if(g_robot2py_position_control != NULL)
    {   
	...
	system("/Network/Servers/duerer/guest/prog/masterUser/src/py/py2robot_position_control.py &");
  //end code
 ...
 *****************************************************************************/
static int 
run_python_task_position_control(void)
{
  int j, i;
  static int count=0;
  ...
  float currentAngularVel;
    ...
    sscanf(g_py2robot_position_control, "%f", &tmpdata); // read elbow Force from python?
 ...
 *****************************************************************************/
static int 
change_python_task_position_control(void)
{
  int    ivar;
  double dvar;
